...
//        publishPolicies(policyList);
    ...
    public List<Agent> executeCPAC(InitialAccessRequest iar,
                                   ...
                                   String m_cur, Vertex v_cur, Vertex v_next) throws Exception {
                ...
                for (SODPolicyRule sod:applicableSoDs)  //  line 25
                {
                    ...
                    if(unsafePermissions.size()>=sod.getK())    //  line 26
                    {
                        if(m_cur.equalsIgnoreCase("emergency"))     //  line 27
                        {
                            ...
                            {
                                authorizedAgent = evaluate(iar, m_cur);//  line 29
                            }
                        }
                    }
                    else
                    {
                        authorizedAgent=evaluate(iar,m_cur);
                    }
                }
            }
            else    //  line 30
            {
                authorizedAgent=evaluate(iar,m_cur);      //  line 31
            }

            if(authorizedAgent!=null)
            {
                agentList_v_cur.add(authorizedAgent);
            }
        }
